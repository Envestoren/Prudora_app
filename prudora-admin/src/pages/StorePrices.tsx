...
export function StorePrices() {
  ...
  const [historyProfilesByUserId, setHistoryProfilesByUserId] = useState<Record<string, { first_name: string; last_name: string; email: string | null }>>({})
  ...
  async function load() {
    ...
    const [pricesRes, storesRes, productsRes] = await Promise.all([
      supabase
        .from('product_prices')
        .select('id, product_id, store_id, user_id, price_amount, recorded_at, approval_status, approved_by, approved_at, products(id,name,unit,barcode), stores(id,chain,name)')
        .order('recorded_at', { ascending: false }),
      supabase.from('stores').select('id, chain, name, address, latitude, longitude, logo_url, created_at, updated_at').order('chain'),
      supabase
        .from('products')
        .select('id, name, supplier, manufacturer, unit, unit_price_amount, is_weight_item, category_id, image_url, barcode, approval_status, submitted_by, submitted_at, approved_by, approved_at, created_at, updated_at')
        .order('name'),
    ])

    if (pricesRes.error || storesRes.error || productsRes.error) {
      setError(pricesRes.error?.message ?? storesRes.error?.message ?? productsRes.error?.message ?? 'Kunne ikke hente priser.')
      setPrices([])
      setStores([])
      setProducts([])
      setLoading(false)
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((p) => `${p.name} ${p.barcode ?? ''} ${p.supplier} ${p.manufacturer}`.toLowerCase().includes(q))
  ...
  async function deletePrice(row: PriceRow) {
    if (!confirm('Slette nyeste prisregistrering? Forrige registrerte pris for samme produkt/butikk blir da vist i matrisen.')) return
    setSaving(true)
    setError(null)
    ...
    <div className="products-page">
      ...
      {loading ? (
        ...
        <>
          <div
            className="card"
            style={{ maxWidth: '100%', padding: 16, marginBottom: 16 }}
          >
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              <strong>Butikker i matrise</strong>
              <span style={{ fontSize: 13, opacity: 0.8 }}>
                ({selectedStoreIds.length} valgt)
              </span>
              <button
                type="button"
                className="btn-page"
                  ...
                  {filteredProducts.map((product) => (
                    <tr key={product.id}>
                      ...
                      {selectedStores.map((store) => {
                        ...
                        const amountText = amount == null || !Number.isFinite(amount) ? '—' : `${amount.toFixed(2)} kr/${product.unit}`
                        const dateText = row?.recorded_at ? new Date(row.recorded_at).toLocaleDateString('nb-NO') : ''
                        return (
                          <td key={store.id}>
                            <div style={{ display: 'flex', gap: 6, alignItems: 'stretch' }}>
                              <button
                                ...
                                title={row ? `Nyeste pris fra ${dateText}. Klikk for å redigere.` : 'Ingen pris registrert. Klikk for å legge til.'}
                              >
                                <div>{amountText}</div>
              ...
              <form onSubmit={saveForm} className="product-form">
                <label>
                  ...
                  {editing ? (
                    <input
                      type="text"
                      value={editing.products?.name ?? 'Ukjent produkt'}
                      readOnly
                    />
                  ) : (
                    <select
                ...
                <div className="form-actions">
                  ...
                  {editing ? (
                    <>
                      ...
                      >
                        Slett nyeste: fjerner bare denne raden og viser forrige pris i matrisen. Slett all historikk: fjerner alle priser for dette produktet i denne butikken.
                      </p>
                      <button
                        type="button"
      ...
      {showHistory ? (
        <div className="category-panel-overlay" onClick={() => setShowHistory(false)} aria-hidden="true">
          <div className="category-panel" onClick={(e) => e.stopPropagation()} role="dialog" aria-label="Prishistorikk">
            ...
            <div className="category-panel-body">
              <p className="section-desc" style={{ marginBottom: 12 }}>{historyTitle}</p>
              {historyLoading ? (
                ...
                <div className="table-wrap">
                  <table>
                    ...
                    <tbody>
                      {historyRows.map((r) => {
                        const amount = typeof r.price_amount === 'number' ? r.price_amount : parseFloat(r.price_amount)
                        ...
                        const approverText = !r.approved_by
                          ...
                          : approver
                            ...
                            : r.approved_by
                        const statusText = r.approval_status === 'approved'
                          ? 'Godkjent'
                        ...
                        return (
                          <tr key={r.id}>
                            <td>{Number.isFinite(amount) ? `${amount.toFixed(2)} kr/${unit}` : `${r.price_amount} kr/${unit}`}</td>
