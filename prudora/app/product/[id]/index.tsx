...
import { Box, HStack, Input, InputField, Pressable, ScrollView, Spinner, Text, VStack } from '@gluestack-ui/themed';
...
import { formatKr } from '@/lib/format-utils';
...
export default function ProductDetailScreen() {
  ...
  // Prisvarsel-abonnement
  type SubData = { alertId: string; percentDrop: number | null; absoluteDropKr: number | null; thresholdPrice: number | null };
  const [sub, setSub] = useState<SubData | null>(null);
  const [subLoading, setSubLoading] = useState(false);
  const [subModalOpen, setSubModalOpen] = useState(false);
  const [subPercentText, setSubPercentText] = useState('');
  const [subAbsoluteText, setSubAbsoluteText] = useState('');
  const [subThresholdPriceText, setSubThresholdPriceText] = useState('');
  ...
  // Hent eksisterende abonnement for dette produktet
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        if (data) {
          ...
          setSubPercentText(data.percent_drop != null ? String(data.percent_drop) : '');
          setSubAbsoluteText(data.absolute_drop_kr != null ? String(data.absolute_drop_kr) : '');
          setSubThresholdPriceText(data.threshold_price != null ? String(data.threshold_price) : '');
        } else {
          setSub(null);
        }
        setSubLoading(false);
  ...
  const sendSubPushNotification = useCallback(async (title: string, body: string) => {
    try {
      ...
      await scheduleNotificationAsync({ content: { title, body, sound: true }, trigger: null });
    } catch {
      // ignorer varslefeil stille
    }
  }, []);

  const subscribeToProduct = useCallback(async () => {
    if (!user || !product) return;
    const { data, error } = await supabase
      .from('user_product_price_alerts')
      .insert({ user_id: user.id, product_id: productId, enabled: true, percent_drop: null, absolute_drop_kr: null })
      .select('id, percent_drop, absolute_drop_kr, threshold_price')
      .single();
    if (error || !data?.id) {
      setSubMessage(error?.message ?? 'Kunne ikke lagre abonnement.');
      return;
    }
    // Sikre tomme grenser ved ny aktivering (unngå uventede default-verdier som 5%).
    if (data.percent_drop != null || data.absolute_drop_kr != null || data.threshold_price != null) {
      await supabase
        .from('user_product_price_alerts')
        .update({ percent_drop: null, absolute_drop_kr: null, threshold_price: null, enabled: true })
        .eq('id', data.id as string);
    }

    setSub({ alertId: data.id as string, percentDrop: null, absoluteDropKr: null, thresholdPrice: null });
    setSubMessage(null);
    // Ikke send push ved aktivering av varsel; kun ved faktiske prisvarsler.
  }, [user, product, productId]);

  const unsubscribeFromProduct = useCallback(async () => {
    if (!sub) return;
    const { error } = await supabase.from('user_product_price_alerts').delete().eq('id', sub.alertId);
    if (error) {
      setSubMessage(error.message ?? 'Kunne ikke fjerne abonnement.');
      return;
    }
    setSub(null);
    setSubPercentText('');
    setSubAbsoluteText('');
    setSubThresholdPriceText('');
    setSubMessage(null);
    setSubModalOpen(false);
  }, [sub]);

  const saveSubThresholds = useCallback(async () => {
    if (!sub) return;
    const percent = parseNullableNumber(subPercentText);
    const absolute = parseNullableNumber(subAbsoluteText);
    const thresholdPrice = parseNullableNumber(subThresholdPriceText);
    if (percent == null && absolute == null && thresholdPrice == null) {
      setSubMessage('Legg inn minst én regel: prosent, kroner eller makspris.');
      return;
    }
    setSubSaving(true);
    const { error } = await supabase
      .from('user_product_price_alerts')
      .update({ percent_drop: percent, absolute_drop_kr: absolute, threshold_price: thresholdPrice, enabled: true })
      .eq('id', sub.alertId);
    setSubSaving(false);
    if (error) {
      setSubMessage(error.message ?? 'Kunne ikke lagre varselgrenser.');
      return;
    }
    setSub((prev) => (prev ? { ...prev, percentDrop: percent, absoluteDropKr: absolute, thresholdPrice } : prev));
    setSubMessage('Varselgrenser lagret!');
    // Ikke send push ved lagring av regler; kun ved faktiske prisvarsler.
  }, [sub, subPercentText, subAbsoluteText, subThresholdPriceText, parseNullableNumber]);
  ...
  return (
    <BlurStatusBarView edges={['top']}>
          ...
          <HStack alignItems="center" justifyContent="space-between">
            ...
            >
              {subLoading ? (
                ...
                <IconSymbol name="bell.fill" size={22} color={sub ? c.primary : c.textSecondary} />
              )}
            </Pressable>
          </HStack>
        </Box>
      ...
      {subModalOpen && (
        <Pressable
          onPress={() => { setSubModalOpen(false); setSubMessage(null); }}
          style={{
            position: 'absolute',
            top: 0,
            right: 0,
                ...
                {sub ? (
                  <>
                    <HStack alignItems="center" style={{ gap: spacing.sm }}>
                      ...
                      <Input size="sm" variant="outline" style={{ flex: 1, borderRadius: radius.md, borderWidth: hairlineWidth }}>
                        <InputField
                          value={subPercentText}
                          onChangeText={setSubPercentText}
                      ...
                      <Box style={{ minWidth: 40, height: 36, borderRadius: radius.md, borderWidth: hairlineWidth, borderColor: c.border, backgroundColor: c.surface, alignItems: 'center', justifyContent: 'center' }}>
                        <Text fontSize={12} fontWeight="700" style={{ color: c.textSecondary }}>
                          %
                        </Text>
                      </Box>
                    ...
                    <HStack alignItems="center" style={{ gap: spacing.sm }}>
                      ...
                      <Input size="sm" variant="outline" style={{ flex: 1, borderRadius: radius.md, borderWidth: hairlineWidth }}>
                        <InputField
                          value={subAbsoluteText}
                          onChangeText={setSubAbsoluteText}
                          placeholder="f.eks. 5"
                          placeholderTextColor={c.textMuted}
                          keyboardType="decimal-pad"
                          style={{ color: c.text }}
                    ...
                    <HStack alignItems="center" style={{ gap: spacing.sm }}>
                      ...
                      <Input size="sm" variant="outline" style={{ flex: 1, borderRadius: radius.md, borderWidth: hairlineWidth }}>
                        <InputField
                          value={subThresholdPriceText}
                          onChangeText={setSubThresholdPriceText}
                          placeholder={`f.eks. ${product ? formatKr(Math.round((product as any).unit_price_amount ?? 0)) : '50'}`}
                          placeholderTextColor={c.textMuted}
                          keyboardType="decimal-pad"
                          style={{ color: c.text }}
                        />
                    ...
                    {subMessage ? (
                      <Text
                        fontSize={13}
                        style={{ color: subMessage.includes('lagret') ? c.primary : '#DC2626' }}
                      >
                        {subMessage}
                      </Text>
                    ) : null}
                    <HStack space="sm" mt={spacing.xs}>
                      <PremiumButton
                        title="Fjern varsel"
                        variant="outline"
                        onPress={() => void unsubscribeFromProduct()}
                        style={{ flex: 1, minHeight: 40 }}
                        textStyle={{ fontSize: 14 }}
                      />
                      <PremiumButton
                        title={subSaving ? 'Lagrer...' : 'Lagre'}
                        onPress={() => {
                          Keyboard.dismiss();
                          void saveSubThresholds();
                        }}
                        disabled={subSaving}
                        style={{ flex: 1, minHeight: 40 }}
                        textStyle={{ fontSize: 14 }}
                  ...
                  <>
                    <Text fontSize={14} style={{ color: c.textMuted, lineHeight: 20 }}>
                      Aktiver for å motta varsler når prisen på dette produktet endres.
                    </Text>
                    {subMessage ? (
                      <Text fontSize={13} style={{ color: '#DC2626' }}>{subMessage}</Text>
                    ) : null}
                    <PremiumButton
                      title="Aktiver prisvarsel"
                      onPress={() => void subscribeToProduct()}
                      style={{ minHeight: 40, marginTop: spacing.xs }}
                      textStyle={{ fontSize: 14 }}
                    />
                  </>
                )}
