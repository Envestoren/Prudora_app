...
export default function ProductSubscriptionScreen() {
  ...
  const addSubscription = useCallback(
    async (product: ProductPick) => {
      ...
      await sendPushNotification(
        'Prudora – Abonnement aktivert',
        `Du abonnerer nå på prisvarsler for ${product.name}.`
      );
    },
    [subscriptions, user, sendPushNotification]
  );
  ...
  const saveThresholds = useCallback(
    async (
      ...
      await sendPushNotification(
        'Prudora – Varselgrense lagret',
        `Varselgrenser for ${item.name} er aktivert.`
      );
    },
    [parseNullableNumber, sendPushNotification]
  );
...
function SubscriptionCard({
  ...
  const [percentText, setPercentText] = useState(
    subscription?.percentDrop != null ? String(subscription.percentDrop).replace('.', ',') : ''
  );
  const [absoluteText, setAbsoluteText] = useState(
    subscription?.absoluteDropKr != null ? String(subscription.absoluteDropKr).replace('.', ',') : ''
  );
  const [thresholdPriceText, setThresholdPriceText] = useState(
    subscription?.thresholdPrice != null ? String(subscription.thresholdPrice).replace('.', ',') : ''
  );
    ...
    <Box p={spacing.md} borderRadius={12} style={{ backgroundColor: c.card ?? c.background, borderWidth: 1, borderColor: c.border }}>
      <HStack space="md" alignItems="flex-start">
        ...
        <VStack flex={1} space="xs">
          ...
          {subscription && onSaveThresholds ? (
            <VStack space="sm" mt={spacing.sm}>
              {!isEditingThresholds ? (
                <PremiumButton
                  title="Rediger varselgrense"
                ...
                <>
                  <HStack alignItems="center" justifyContent="space-between">
                    ...
                    <Pressable
                      onPress={() => setShowThresholdInfo((prev) => !prev)}
                      style={[
                        styles.infoIconButton,
                        {
                          borderColor: showThresholdInfo ? (c.tint ?? c.border) : c.border,
                          backgroundColor: showThresholdInfo ? (c.tint ?? c.border) : c.surface,
                        },
                      ]}
                    >
                      <Text
                        fontSize={12}
                        fontWeight="700"
                        style={{ color: showThresholdInfo ? c.background : c.textSecondary }}
                      >
                        i
                      </Text>
                    </Pressable>
                  ...
                  <Text fontSize={12} fontWeight="600" style={{ color: c.textMuted }}>
                    Varsle også hvis pris er under
                  </Text>
                  <HStack alignItems="center" style={{ gap: spacing.sm }}>
                    <Input size="sm" variant="outline" style={[styles.thresholdInput, { flex: 1 }]}>
                      <InputField
                        value={thresholdPriceText}
                        onChangeText={setThresholdPriceText}
                  ...
                  <HStack space="sm">
                    <Box flex={1}>
                      <PremiumButton
                        title="Lagre"
                        onPress={() => {
                          void onSaveThresholds(subscription, percentText, absoluteText, thresholdPriceText);
